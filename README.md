...
|---|---|
| **Cell names** | Every cell has a unique kebab-case name stored in `cell.metadata.jupyter_mcp.name`. Unnamed cells adopt their ipynb cell id when it is human-meaningful (`fetch-gtfs`, not auto-generated hex), else get auto-names from their first comment/heading. |
...
| **Condensed outputs** | Streams merged, ANSI stripped, long text truncated head+tail with explicit markers. Duplicate table reprs collapse to one: uniform → CSV, ragged → JSON. Table labels count data rows only and call out truncated previews (`showing 10 of 29,658 rows`). Charts return as real MCP images (downscaled), so the agent *sees* them. |
| **Summaries** | Lazy, batched `claude-haiku-4-5` summaries (tldr + description + output summary) cached in cell metadata keyed by content hash. Unchanged cells are never re-summarized. LLM summaries are marked `~` (approximate); deterministic fallbacks (used without API credentials) are marked `*`. |
...
| `run` | Default: all stale cells (minimal recompute). With `cells`: exactly those, even if fresh, after freshening their stale ancestors (`fresh_deps`). Per-cell wall-clock timing in results. Bounded wait; continues in background |
| `configure_notebook` | Notebook-level defaults for `run`'s `timeout_seconds` / `wait_seconds` |
